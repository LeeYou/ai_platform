...
| Phase 8 | 样本标注子系统 | 第 34-38 周 | 标注后端/前端、多类型标注、导出 |
...
## Phase 8: 样本标注子系统 (Weeks 34-38)

> **目标**：在训练容器中增加完整的样本标注功能，形成"标注→训练→测试→授权→编译→生产→迭代"闭环。

**8.1 标注后端**

- [x] AnnotationProject / AnnotationRecord 数据模型（SQLAlchemy ORM）
- [x] Pydantic v2 请求/响应 Schema（含 JSON 校验）
- [x] CRUD 操作（项目 + 标注记录）
- [x] 标注项目管理 API（CRUD + 统计 + 导出）
- [x] 样本列表 API（文件扫描 + 标注状态）
- [x] 标注记录 API（创建/更新/删除）
- [x] 图片服务 API（路径安全校验，防目录遍历）
- [x] 多格式导出（分类目录/YOLO/OCR/通用JSON）

**8.2 标注前端**

- [x] 标注项目列表页（AnnotationProjects.vue）
  - 项目 CRUD 对话框
  - 关联 AI 能力选择
  - 标注类型 / 网络选型选择
  - 标签配置 JSON 编辑器
  - 进度条展示
  - 导出功能
- [x] 标注工作台页（AnnotationWorkspace.vue）
  - 二分类标注工具（按钮 + 快捷键 0/1）
  - 多分类标注工具（标签面板 + 数字键快捷键）
  - 目标检测标注工具（Canvas 矩形框绘制 + 标签选择）
  - OCR 标注工具（Canvas 区域绘制 + 文字输入）
  - 图像分割标注工具
  - 样本导航（上/下一张 + 跳转 + 过滤）
  - 键盘快捷键（方向键翻页）
  - 自动跳转下一未标注样本
...
- [x] `docs/design/annotation_service.md` 标注子系统设计文档
- [x] 更新 `docs/design/architecture.md` 系统架构文档
- [x] 更新 `docs/design/train_service.md` 训练子系统设计文档

...
### Layer 1：HTTP 服务层校验（路由级）

- 服务启动时加载并校验 License
- 每次推理请求前检查 License 状态（结果缓存 60 秒，减少 IO）
- 校验内容：签名有效性、有效期、能力范围、机器指纹匹配
- **到期行为**：禁止推理接口（返回 4002），保留 `/health` 和 `/license/status` 可正常访问

### Layer 2：SO 插件层校验（能力级）

- 每个 SO 内部在 `AiInit()` 时做完整签名验证
- 推理时每隔 N 次（默认 1000 次）做轻量校验（时间戳检查）
- SO 内置公钥（编译时硬编码），无法从外部替换
- 即使绕过 HTTP 层直接调用 SO，也无法通过能力级校验

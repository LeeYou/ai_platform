...
授权子系统是独立的授权管理平台，负责为客户生成、管理和校验 AI 能力使用授权。授权文件使用 RSA-2048-PSS + SHA-256 数字签名（PSS 填充，MGF1-SHA256），支持机器硬件绑定、试用期限和长期授权等模式。
...
```json
{
  ...
  "signature": "BASE64(RSA-PSS-SHA256-SIGN(上述全部字段的规范化JSON字节))"
}
```
...
| `operating_system` | enum | 目标操作系统，支持 `windows` / `linux` / `android` / `ios` |
| `minimum_os_version` | string | 最低系统版本；为空/`null` 表示不限制 |
| `system_architecture` | string | 目标系统架构；为空/`null` 表示不限制 |
| `application_name` | string | 应用名称，仅作识别和审计展示，不参与准入判定 |
...
- `operating_system` 为新签发授权的必填字段，用于限定授权运行的平台类型。
- `minimum_os_version` 为可选字段；仅当填写时，Runtime 和授权工具需要校验当前系统版本不低于该值。
- `system_architecture` 为可选字段；为空时不限制，填写后要求运行环境架构与授权一致。
- `application_name` 仅作授权管理后台、交付清单和问题排查时的识别信息，不参与 Runtime 准入决策。
- 为兼容历史授权，旧授权文件缺失上述字段时按“不限制”处理；新签发授权必须写入 `operating_system` 和 `application_name`。
...
- SO 侧校验内容至少包括：签名有效性、有效期、能力范围、机器指纹匹配（如配置）、操作系统、最低系统版本、系统架构、版本约束、实例数上限。
...
3. 勾选授权的 AI 能力范围
4. 选择目标操作系统，填写应用名称
5. 选择最低系统版本（可选）和系统架构（可选）
6. 输入目标机器指纹（由 `license_tool fingerprint` 采集）
7. 设置并发实例数上限
8. 点击生成 → 系统使用私钥签名 → 输出 `license.bin`
9. 下载 License 文件，随交付包一起交付给客户
...
| 模块 | 改造点 |
|------|--------|
| 授权后端 | 扩展 Schema / 数据表 / 签名载荷 / SQLite 迁移逻辑，支持新增字段签发与查询 |
| 授权前端 | 扩展授权生成与授权列表页面，确保新字段可录入、可校验、可回显 |
| license_tool / license_core | 扩展模板解析、签名载荷与本地校验，支持环境约束字段 |
| C++ Runtime | 新增操作系统、最低系统版本、系统架构匹配校验，并向 Web 层暴露详细状态 |
| 项目文档 | 维护设计说明、开发计划、阶段进度和阶段小结，保证进度可跟踪 |

> 本次增强的详细开发计划与实际进度见：`/home/runner/work/ai_platform/ai_platform/docs/license_authorization_extension_plan.md`

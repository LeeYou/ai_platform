...
nlohmann::json inferInternal(Context* ctx, const cv::Mat& image, int32_t* error_out) {
    ...
    const float real_confidence = ctx->vision->detectLegacyLiveConfidence(work, det.face_rect);
    float fake_confidence = agface::clamp01(1.0f - real_confidence);
    ...
    result["status"] = 1;
    result["result"] = fake_confidence >= 0.58f ? 1 : 0;
    result["confidence"] = fake_confidence;
...
bool initInternal(Context* ctx, std::string* err) {
    ...
    if (ctx->vision->initFakePhotoModels(ctx->model_dir, 2)) {
        return true;
    }
    ctx->vision.reset();
    if (err) *err = "failed to init legacy fake_photo models";
    return false;
}
...
AI_EXPORT int32_t AiInit(AiHandle handle) {
    ...
    if (!initInternal(ctx, &err)) {
        std::fprintf(stderr, "[agface_fake_photo] init failed: %s\n", err.c_str());
...
AI_EXPORT int32_t AiInfer(AiHandle handle, const AiImage* input, AiResult* output) {
    ...
    auto* ctx = static_cast<Context*>(handle);
    if (!ctx->initialized) {
        agface::fillError(output, AI_ERR_INVALID_PARAM, "agface_fake_photo not initialized");
        return AI_ERR_INVALID_PARAM;
    }

    cv::Mat bgr;
    if (!agface::aiImageToBgrMat(input, &bgr) || bgr.empty()) {
        agface::fillError(output, AI_ERR_IMAGE_DECODE, "agface_fake_photo: invalid AiImage");
    ...
    try {
        ...
        if (err_code != AI_OK) {
            agface::fillResult(output, err_code, payload, "agface_fake_photo: inference error");
            return err_code;
        }
        ...
        agface::fillError(output, AI_ERR_INTERNAL, "agface_fake_photo: unknown exception");
        return AI_ERR_INTERNAL;
    }
}
...
AI_EXPORT int32_t AiReload(AiHandle handle, const char* new_model_dir) {
    ...
    if (!initInternal(&staging, &err)) {
        std::fprintf(stderr, "[agface_fake_photo] reload failed: %s\n", err.c_str());
        return AI_ERR_LOAD_FAILED;
    }

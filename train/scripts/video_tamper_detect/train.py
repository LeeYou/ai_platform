"""Train video_tamper_detect (视频篡改检测).

Architecture: CNN+时序分析

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/video_tamper_detect/ \\
                    --output /workspace/models/video_tamper_detect/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train video_tamper_detect")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/video_tamper_detect/")
    p.add_argument("--output",  default="/workspace/models/video_tamper_detect/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real CNN+时序分析 model."""
...
def main():
    ...
    print(f"[INFO] Starting training: video_tamper_detect v{args.version}", flush=True)

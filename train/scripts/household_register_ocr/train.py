"""Train household_register_ocr (户口本识别).

Architecture: PP-OCRv4+KIE

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/household_register_ocr/ \\
                    --output /workspace/models/household_register_ocr/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train household_register_ocr")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/household_register_ocr/")
    p.add_argument("--output",  default="/workspace/models/household_register_ocr/v1.0.0/")
...
def main():
    ...
    print(f"[INFO] Starting training: household_register_ocr v{args.version}", flush=True)

"""Train doc_rectify (文档矫正（倾斜文档自动摆正）).

Architecture: DocTr/MORAN

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/doc_rectify/ \\
                    --output /workspace/models/doc_rectify/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train doc_rectify")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/doc_rectify/")
    p.add_argument("--output",  default="/workspace/models/doc_rectify/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real DocTr/MORAN model."""
...
def main():
    ...
    print(f"[INFO] Starting training: doc_rectify v{args.version}", flush=True)

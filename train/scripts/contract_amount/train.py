"""Train contract_amount (合同额识别).

Architecture: BERT+NER

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/contract_amount/ \\
                    --output /workspace/models/contract_amount/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train contract_amount")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/contract_amount/")
    p.add_argument("--output",  default="/workspace/models/contract_amount/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real BERT+NER model."""
...
def main():
    ...
    print(f"[INFO] Starting training: contract_amount v{args.version}", flush=True)

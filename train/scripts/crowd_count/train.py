"""Train crowd_count (人流量统计).

Architecture: CSRNet/DM-Count

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/crowd_count/ \\
                    --output /workspace/models/crowd_count/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train crowd_count")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/crowd_count/")
    p.add_argument("--output",  default="/workspace/models/crowd_count/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real CSRNet/DM-Count model."""
...
def main():
    ...
    print(f"[INFO] Starting training: crowd_count v{args.version}", flush=True)

"""Train ocr_business_license (营业执照识别).

Architecture: PP-OCRv4+KIE

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/ocr_business_license/ \\
                    --output /workspace/models/ocr_business_license/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train ocr_business_license")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/ocr_business_license/")
    p.add_argument("--output",  default="/workspace/models/ocr_business_license/v1.0.0/")
...
def main():
    ...
    print(f"[INFO] Starting training: ocr_business_license v{args.version}", flush=True)

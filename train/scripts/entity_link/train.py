"""Train entity_link (知识图谱实体链接).

Architecture: ELMo/BERT+知识库

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/entity_link/ \\
                    --output /workspace/models/entity_link/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train entity_link")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/entity_link/")
    p.add_argument("--output",  default="/workspace/models/entity_link/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real ELMo/BERT+知识库 model."""
...
def main():
    ...
    print(f"[INFO] Starting training: entity_link v{args.version}", flush=True)

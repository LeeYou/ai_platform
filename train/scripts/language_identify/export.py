"""Export language_identify (语种识别) model to ONNX format.

Usage:
    python export.py --output /workspace/models/language_identify/v1.0.0/ --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Export language_identify to ONNX")
...
def _write_preprocess_json(output_path: str) -> None:
    data = {
        "resize": {"width": 80, "height": 300, "keep_ratio": True},
...
def _write_labels_json(output_path: str) -> None:
    data = {"labels": ['zh', 'en', 'ja', 'ko', 'fr', 'de', 'ru', 'ar', 'es'], "num_classes": 9}
...
def main():
    ...
    # TODO: replace with real Whisper/VoxLingua107 model class

"""Train face_swap (人脸融合（换脸）).

Architecture: SimSwap/FaceShifter

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/face_swap/ \\
                    --output /workspace/models/face_swap/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train face_swap")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/face_swap/")
    p.add_argument("--output",  default="/workspace/models/face_swap/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real SimSwap/FaceShifter model."""
...
def main():
    ...
    print(f"[INFO] Starting training: face_swap v{args.version}", flush=True)

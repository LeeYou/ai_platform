"""Train image_enhance (图像对比度增强).

Architecture: Zero-DCE/RetinexNet

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/image_enhance/ \\
                    --output /workspace/models/image_enhance/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train image_enhance")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/image_enhance/")
    p.add_argument("--output",  default="/workspace/models/image_enhance/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real Zero-DCE/RetinexNet model."""
...
def main():
    ...
    print(f"[INFO] Starting training: image_enhance v{args.version}", flush=True)

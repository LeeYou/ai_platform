"""Train video_track (视频目标跟踪).

Architecture: ByteTrack/DeepSORT

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/video_track/ \\
                    --output /workspace/models/video_track/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train video_track")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/video_track/")
    p.add_argument("--output",  default="/workspace/models/video_track/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real ByteTrack/DeepSORT model."""
...
def main():
    ...
    print(f"[INFO] Starting training: video_track v{args.version}", flush=True)

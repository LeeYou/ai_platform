"""Train seal_recognize (印章识别与提取).

Architecture: YOLOv8n+分类头

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/seal_recognize/ \\
                    --output /workspace/models/seal_recognize/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train seal_recognize")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/seal_recognize/")
    p.add_argument("--output",  default="/workspace/models/seal_recognize/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real YOLOv8n+分类头 model."""
...
def main():
    ...
    print(f"[INFO] Starting training: seal_recognize v{args.version}", flush=True)

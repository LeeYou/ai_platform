"""Train video_frame_extract (视频帧提取).

Architecture: FFmpeg+质量评估

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/video_frame_extract/ \\
                    --output /workspace/models/video_frame_extract/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train video_frame_extract")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/video_frame_extract/")
    p.add_argument("--output",  default="/workspace/models/video_frame_extract/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real FFmpeg+质量评估 model."""
...
def main():
    ...
    print(f"[INFO] Starting training: video_frame_extract v{args.version}", flush=True)

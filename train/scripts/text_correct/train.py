"""Train text_correct (文本纠错).

Architecture: MacBERT/SoftMaskedBERT

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/text_correct/ \\
                    --output /workspace/models/text_correct/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train text_correct")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/text_correct/")
    p.add_argument("--output",  default="/workspace/models/text_correct/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real MacBERT/SoftMaskedBERT model."""
...
def main():
    ...
    print(f"[INFO] Starting training: text_correct v{args.version}", flush=True)

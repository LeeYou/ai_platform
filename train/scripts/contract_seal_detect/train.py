"""Train contract_seal_detect (合同印章检测).

Architecture: YOLOv8n

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/contract_seal_detect/ \\
                    --output /workspace/models/contract_seal_detect/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train contract_seal_detect")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/contract_seal_detect/")
    p.add_argument("--output",  default="/workspace/models/contract_seal_detect/v1.0.0/")
...
def main():
    ...
    print(f"[INFO] Starting training: contract_seal_detect v{args.version}", flush=True)

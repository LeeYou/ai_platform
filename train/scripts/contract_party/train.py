"""Train contract_party (合同甲方乙方识别).

Architecture: BERT+NER

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/contract_party/ \\
                    --output /workspace/models/contract_party/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train contract_party")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/contract_party/")
    p.add_argument("--output",  default="/workspace/models/contract_party/v1.0.0/")
...
def main():
    ...
    print(f"[INFO] Starting training: contract_party v{args.version}", flush=True)

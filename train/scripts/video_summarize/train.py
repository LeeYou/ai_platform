"""Train video_summarize (视频摘要).

Architecture: CLIP+TextRank
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train video_summarize")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/video_summarize/")
    p.add_argument("--output",  default="/workspace/models/video_summarize/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real CLIP+TextRank model."""
...
def main():
    ...
    print(f"[INFO] Starting training: video_summarize v{args.version}", flush=True)

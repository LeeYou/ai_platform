"""Train image_retrieval (图像相似度检索).

Architecture: ViT-B/16+CLIP

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/image_retrieval/ \\
                    --output /workspace/models/image_retrieval/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train image_retrieval")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/image_retrieval/")
    p.add_argument("--output",  default="/workspace/models/image_retrieval/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real ViT-B/16+CLIP model."""
...
def main():
    ...
    print(f"[INFO] Starting training: image_retrieval v{args.version}", flush=True)

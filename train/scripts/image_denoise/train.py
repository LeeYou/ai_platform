"""Train image_denoise (图像去噪).

Architecture: DnCNN/NAFNet

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/image_denoise/ \\
                    --output /workspace/models/image_denoise/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train image_denoise")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/image_denoise/")
    p.add_argument("--output",  default="/workspace/models/image_denoise/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real DnCNN/NAFNet model."""
...
def main():
    ...
    print(f"[INFO] Starting training: image_denoise v{args.version}", flush=True)

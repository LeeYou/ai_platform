"""Train text_classify (文本分类).

Architecture: BERT/TextCNN

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/text_classify/ \\
                    --output /workspace/models/text_classify/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train text_classify")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/text_classify/")
    p.add_argument("--output",  default="/workspace/models/text_classify/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real BERT/TextCNN model."""
...
def main():
    ...
    print(f"[INFO] Starting training: text_classify v{args.version}", flush=True)

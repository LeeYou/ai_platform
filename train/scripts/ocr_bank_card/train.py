"""Train ocr_bank_card (银行卡识别).

Architecture: CRNN+PP-OCRv4

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/ocr_bank_card/ \\
                    --output /workspace/models/ocr_bank_card/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train ocr_bank_card")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/ocr_bank_card/")
    p.add_argument("--output",  default="/workspace/models/ocr_bank_card/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real CRNN+PP-OCRv4 model."""
...
def main():
    ...
    print(f"[INFO] Starting training: ocr_bank_card v{args.version}", flush=True)

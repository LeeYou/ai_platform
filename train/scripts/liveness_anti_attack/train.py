"""Train liveness_anti_attack (活体防攻击检测).

Architecture: MiniFASNet+多模态融合

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/liveness_anti_attack/ \\
                    --output /workspace/models/liveness_anti_attack/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train liveness_anti_attack")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/liveness_anti_attack/")
    p.add_argument("--output",  default="/workspace/models/liveness_anti_attack/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real MiniFASNet+多模态融合 model."""
    import torch.nn as nn
    return nn.Sequential(
        nn.Flatten(),
        nn.Linear(150528, 256),
        nn.ReLU(),
        nn.Linear(256, max(1, 5)),
    )
...
def main():
    ...
    print(f"[INFO] Starting training: liveness_anti_attack v{args.version}", flush=True)

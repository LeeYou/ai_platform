"""Train ocr_vehicle_license (行驶证识别).

Architecture: PP-OCRv4+KIE

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/ocr_vehicle_license/ \\
                    --output /workspace/models/ocr_vehicle_license/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train ocr_vehicle_license")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/ocr_vehicle_license/")
    p.add_argument("--output",  default="/workspace/models/ocr_vehicle_license/v1.0.0/")
...
def main():
    ...
    print(f"[INFO] Starting training: ocr_vehicle_license v{args.version}", flush=True)

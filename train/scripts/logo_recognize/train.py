"""Train logo_recognize (商标Logo识别).

Architecture: EfficientNet-B3+ArcFace

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/logo_recognize/ \\
                    --output /workspace/models/logo_recognize/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train logo_recognize")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/logo_recognize/")
    p.add_argument("--output",  default="/workspace/models/logo_recognize/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real EfficientNet-B3+ArcFace model."""
...
def main():
    ...
    print(f"[INFO] Starting training: logo_recognize v{args.version}", flush=True)

"""Train person_reid (行人重识别（Person ReID）).

Architecture: OSNet/TransReID

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/person_reid/ \\
                    --output /workspace/models/person_reid/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train person_reid")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/person_reid/")
    p.add_argument("--output",  default="/workspace/models/person_reid/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real OSNet/TransReID model."""
...
def main():
    ...
    print(f"[INFO] Starting training: person_reid v{args.version}", flush=True)

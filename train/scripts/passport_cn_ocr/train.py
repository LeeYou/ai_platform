"""Train passport_cn_ocr (中国护照识别).

Architecture: PP-OCRv4+KIE

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/passport_cn_ocr/ \\
                    --output /workspace/models/passport_cn_ocr/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train passport_cn_ocr")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/passport_cn_ocr/")
    p.add_argument("--output",  default="/workspace/models/passport_cn_ocr/v1.0.0/")
...
def main():
    ...
    print(f"[INFO] Starting training: passport_cn_ocr v{args.version}", flush=True)

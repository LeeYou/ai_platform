"""Train scene_text_detect (场景文字检测).

Architecture: DBNet/EAST

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/scene_text_detect/ \\
                    --output /workspace/models/scene_text_detect/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train scene_text_detect")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/scene_text_detect/")
    p.add_argument("--output",  default="/workspace/models/scene_text_detect/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real DBNet/EAST model."""
    import torch.nn as nn
    return nn.Sequential(
        nn.Flatten(),
        nn.Linear(1228800, 256),
        nn.ReLU(),
        nn.Linear(256, max(1, 1)),
    )
...
def main():
    ...
    print(f"[INFO] Starting training: scene_text_detect v{args.version}", flush=True)

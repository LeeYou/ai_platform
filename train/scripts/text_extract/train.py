"""Train text_extract (文本结构化抽取（合同/公文）).

Architecture: LayoutLMv3/BERT+UIE

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/text_extract/ \\
                    --output /workspace/models/text_extract/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train text_extract")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/text_extract/")
    p.add_argument("--output",  default="/workspace/models/text_extract/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real LayoutLMv3/BERT+UIE model."""
...
def main():
    ...
    print(f"[INFO] Starting training: text_extract v{args.version}", flush=True)

"""Train image_super_res (图像超分辨率).

Architecture: ESRGAN/Real-ESRGAN

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/image_super_res/ \\
                    --output /workspace/models/image_super_res/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train image_super_res")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/image_super_res/")
    p.add_argument("--output",  default="/workspace/models/image_super_res/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real ESRGAN/Real-ESRGAN model."""
...
def main():
    ...
    print(f"[INFO] Starting training: image_super_res v{args.version}", flush=True)

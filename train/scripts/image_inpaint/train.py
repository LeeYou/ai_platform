"""Train image_inpaint (图像修复).

Architecture: LaMa/DeepFill
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train image_inpaint")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/image_inpaint/")
    p.add_argument("--output",  default="/workspace/models/image_inpaint/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real LaMa/DeepFill model."""
...
def main():
    ...
    print(f"[INFO] Starting training: image_inpaint v{args.version}", flush=True)

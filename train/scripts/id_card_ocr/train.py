"""Train id_card_ocr (身份证全字段OCR).

Architecture: PP-OCRv4+KIE

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/id_card_ocr/ \\
                    --output /workspace/models/id_card_ocr/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train id_card_ocr")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/id_card_ocr/")
    p.add_argument("--output",  default="/workspace/models/id_card_ocr/v1.0.0/")
...
def main():
    ...
    print(f"[INFO] Starting training: id_card_ocr v{args.version}", flush=True)

"""Train id_card_back_detect (身份证国徽面检测).

Architecture: YOLOv8n

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/id_card_back_detect/ \\
                    --output /workspace/models/id_card_back_detect/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train id_card_back_detect")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/id_card_back_detect/")
    p.add_argument("--output",  default="/workspace/models/id_card_back_detect/v1.0.0/")
...
def main():
    ...
    print(f"[INFO] Starting training: id_card_back_detect v{args.version}", flush=True)

"""Train text_segment (文本分词).

Architecture: Jieba/LTP/BERT-CRF

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/text_segment/ \\
                    --output /workspace/models/text_segment/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train text_segment")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/text_segment/")
    p.add_argument("--output",  default="/workspace/models/text_segment/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real Jieba/LTP/BERT-CRF model."""
...
def main():
    ...
    print(f"[INFO] Starting training: text_segment v{args.version}", flush=True)

"""Train ocr_handwriting (手写体OCR).

Architecture: TrOCR/CRNN

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/ocr_handwriting/ \\
                    --output /workspace/models/ocr_handwriting/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train ocr_handwriting")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/ocr_handwriting/")
    p.add_argument("--output",  default="/workspace/models/ocr_handwriting/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real TrOCR/CRNN model."""
...
def main():
    ...
    print(f"[INFO] Starting training: ocr_handwriting v{args.version}", flush=True)

"""Train speaker_diarize (说话人分离).

Architecture: pyannote.audio/NSD

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/speaker_diarize/ \\
                    --output /workspace/models/speaker_diarize/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train speaker_diarize")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/speaker_diarize/")
    p.add_argument("--output",  default="/workspace/models/speaker_diarize/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real pyannote.audio/NSD model."""
    import torch.nn as nn
    return nn.Sequential(
        nn.Flatten(),
        nn.Linear(720000, 256),
        nn.ReLU(),
...
def main():
    ...
    print(f"[INFO] Starting training: speaker_diarize v{args.version}", flush=True)

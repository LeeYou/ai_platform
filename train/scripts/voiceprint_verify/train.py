"""Train voiceprint_verify (声纹识别（1:1比对）).

Architecture: ECAPA-TDNN/d-vector

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/voiceprint_verify/ \\
                    --output /workspace/models/voiceprint_verify/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train voiceprint_verify")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/voiceprint_verify/")
    p.add_argument("--output",  default="/workspace/models/voiceprint_verify/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real ECAPA-TDNN/d-vector model."""
...
def main():
    ...
    print(f"[INFO] Starting training: voiceprint_verify v{args.version}", flush=True)

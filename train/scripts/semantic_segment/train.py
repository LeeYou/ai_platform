"""Train semantic_segment (图像语义分割).

Architecture: DeepLabV3+/SegFormer

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/semantic_segment/ \\
                    --output /workspace/models/semantic_segment/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train semantic_segment")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/semantic_segment/")
    p.add_argument("--output",  default="/workspace/models/semantic_segment/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real DeepLabV3+/SegFormer model."""
...
def main():
    ...
    print(f"[INFO] Starting training: semantic_segment v{args.version}", flush=True)

"""Train body_action_recognize (肢体动作识别).

Architecture: ST-GCN/PoseC3D
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train body_action_recognize")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/body_action_recognize/")
...
def _build_model(device):
    """TODO: Replace with the real ST-GCN/PoseC3D model."""
...
def main():
    ...
    print(f"[INFO] Starting training: body_action_recognize v{args.version}", flush=True)

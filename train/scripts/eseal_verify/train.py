"""Train eseal_verify (电子签章真伪校验).

Architecture: EfficientNet-B2+特征比对

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/eseal_verify/ \\
                    --output /workspace/models/eseal_verify/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train eseal_verify")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/eseal_verify/")
    p.add_argument("--output",  default="/workspace/models/eseal_verify/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real EfficientNet-B2+特征比对 model."""
...
def main():
    ...
    print(f"[INFO] Starting training: eseal_verify v{args.version}", flush=True)

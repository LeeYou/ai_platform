"""Train asr_punct_restore (语音转写标点恢复).

Architecture: BERT+CRF/CT-Transformer

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/asr_punct_restore/ \\
                    --output /workspace/models/asr_punct_restore/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train asr_punct_restore")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/asr_punct_restore/")
    p.add_argument("--output",  default="/workspace/models/asr_punct_restore/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real BERT+CRF/CT-Transformer model."""
...
def main():
    ...
    print(f"[INFO] Starting training: asr_punct_restore v{args.version}", flush=True)

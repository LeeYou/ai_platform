"""Train image_classify (图像分类).

Architecture: EfficientNet-B0/ResNet50

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/image_classify/ \\
                    --output /workspace/models/image_classify/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train image_classify")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/image_classify/")
    p.add_argument("--output",  default="/workspace/models/image_classify/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real EfficientNet-B0/ResNet50 model."""
...
def main():
    ...
    print(f"[INFO] Starting training: image_classify v{args.version}", flush=True)

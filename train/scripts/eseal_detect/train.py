"""Train eseal_detect (电子签章检测).

Architecture: YOLOv8n

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/eseal_detect/ \\
                    --output /workspace/models/eseal_detect/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train eseal_detect")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/eseal_detect/")
    p.add_argument("--output",  default="/workspace/models/eseal_detect/v1.0.0/")
...
def main():
    ...
    print(f"[INFO] Starting training: eseal_detect v{args.version}", flush=True)

"""Train face_3d_reconstruct (3D人脸重建).

Architecture: 3DDFA_V2/Deep3DFace

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/face_3d_reconstruct/ \\
                    --output /workspace/models/face_3d_reconstruct/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train face_3d_reconstruct")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/face_3d_reconstruct/")
    p.add_argument("--output",  default="/workspace/models/face_3d_reconstruct/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real 3DDFA_V2/Deep3DFace model."""
...
def main():
    ...
    print(f"[INFO] Starting training: face_3d_reconstruct v{args.version}", flush=True)

"""Train ocr_print (印刷体OCR).

Architecture: PP-OCRv4 rec

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/ocr_print/ \\
                    --output /workspace/models/ocr_print/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train ocr_print")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/ocr_print/")
    p.add_argument("--output",  default="/workspace/models/ocr_print/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real PP-OCRv4 rec model."""
...
def main():
    ...
    print(f"[INFO] Starting training: ocr_print v{args.version}", flush=True)

"""Train image_tamper_detect (图像篡改检测).

Architecture: MantraNet/MVSS-Net

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/image_tamper_detect/ \\
                    --output /workspace/models/image_tamper_detect/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train image_tamper_detect")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/image_tamper_detect/")
    p.add_argument("--output",  default="/workspace/models/image_tamper_detect/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real MantraNet/MVSS-Net model."""
...
def main():
    ...
    print(f"[INFO] Starting training: image_tamper_detect v{args.version}", flush=True)

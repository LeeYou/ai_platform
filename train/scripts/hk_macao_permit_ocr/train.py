"""Train hk_macao_permit_ocr (港澳台通行证识别).

Architecture: PP-OCRv4+KIE

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/hk_macao_permit_ocr/ \\
                    --output /workspace/models/hk_macao_permit_ocr/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train hk_macao_permit_ocr")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/hk_macao_permit_ocr/")
    p.add_argument("--output",  default="/workspace/models/hk_macao_permit_ocr/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real PP-OCRv4+KIE model."""
    import torch.nn as nn
    return nn.Sequential(
        nn.Flatten(),
        nn.Linear(768000, 256),
        nn.ReLU(),
...
def main():
    ...
    print(f"[INFO] Starting training: hk_macao_permit_ocr v{args.version}", flush=True)

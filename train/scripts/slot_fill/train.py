"""Train slot_fill (槽位填充).

Architecture: BERT-CRF

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/slot_fill/ \\
                    --output /workspace/models/slot_fill/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train slot_fill")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/slot_fill/")
    p.add_argument("--output",  default="/workspace/models/slot_fill/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real BERT-CRF model."""
...
def main():
    ...
    print(f"[INFO] Starting training: slot_fill v{args.version}", flush=True)

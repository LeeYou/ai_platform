"""Train vocal_separate (人声分离).

Architecture: Demucs/Conv-TasNet

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/vocal_separate/ \\
                    --output /workspace/models/vocal_separate/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train vocal_separate")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/vocal_separate/")
    p.add_argument("--output",  default="/workspace/models/vocal_separate/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real Demucs/Conv-TasNet model."""
...
def main():
    ...
    print(f"[INFO] Starting training: vocal_separate v{args.version}", flush=True)

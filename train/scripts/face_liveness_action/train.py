"""Train face_liveness_action (指令活体检测（唇语/动作）).

Architecture: LSTM+CNN/MediaPipe

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/face_liveness_action/ \\
                    --output /workspace/models/face_liveness_action/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train face_liveness_action")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/face_liveness_action/")
    p.add_argument("--output",  default="/workspace/models/face_liveness_action/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real LSTM+CNN/MediaPipe model."""
    import torch.nn as nn
    return nn.Sequential(
        nn.Flatten(),
        nn.Linear(150528, 256),
        nn.ReLU(),
        nn.Linear(256, max(1, 4)),
    )
...
def main():
    ...
    print(f"[INFO] Starting training: face_liveness_action v{args.version}", flush=True)

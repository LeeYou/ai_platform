"""Train face_liveness_silent (静默活体检测).

Architecture: MobileNetV3/MiniFASNet

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/face_liveness_silent/ \\
                    --output /workspace/models/face_liveness_silent/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train face_liveness_silent")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/face_liveness_silent/")
    p.add_argument("--output",  default="/workspace/models/face_liveness_silent/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real MobileNetV3/MiniFASNet model."""
...
def main():
    ...
    print(f"[INFO] Starting training: face_liveness_silent v{args.version}", flush=True)

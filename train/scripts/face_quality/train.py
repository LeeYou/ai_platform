"""Train face_quality (人脸质量评估).

Architecture: MobileNetV2 regression

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/face_quality/ \\
                    --output /workspace/models/face_quality/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train face_quality")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/face_quality/")
    p.add_argument("--output",  default="/workspace/models/face_quality/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real MobileNetV2 regression model."""
...
def main():
    ...
    print(f"[INFO] Starting training: face_quality v{args.version}", flush=True)

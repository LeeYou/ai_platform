"""Train pose_estimate (人体关键点检测（姿态估计）).

Architecture: HRNet-W32/YOLOv8-pose
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train pose_estimate")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/pose_estimate/")
    p.add_argument("--output",  default="/workspace/models/pose_estimate/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real HRNet-W32/YOLOv8-pose model."""
...
def main():
    ...
    print(f"[INFO] Starting training: pose_estimate v{args.version}", flush=True)

"""Export doc_classify (证件类别自动分类) model to ONNX format.

Usage:
    python export.py --output /workspace/models/doc_classify/v1.0.0/ --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Export doc_classify to ONNX")
    p.add_argument("--output",     default="/workspace/models/doc_classify/v1.0.0/")
...
def _write_preprocess_json(output_path: str) -> None:
    data = {
        "resize": {"width": 224, "height": 224, "keep_ratio": True},
...
def _write_labels_json(output_path: str) -> None:
    data = {"labels": ['id_front', 'id_back', 'bank_card', 'driver_license', 'vehicle_license', 'invoice', 'business_license', 'temp_id', 'hk_macao', 'officer', 'household', 'cn_passport', 'intl_passport', 'soldier', 'hk_macao_mainland', 'tw_mainland', 'hk_macao_res', 'tw_res', 'foreigner_perm_res'], "num_classes": 19}
...
def main():
    ...
    # TODO: replace with real EfficientNet-B0 model class
    import torch.nn as nn
    model = nn.Sequential(
        nn.Flatten(),
        nn.Linear(150528, 256),
        nn.ReLU(),
        nn.Linear(256, max(1, 19)),
    )

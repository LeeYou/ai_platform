"""Train doc_classify (证件类别自动分类).

Architecture: EfficientNet-B0

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/doc_classify/ \\
                    --output /workspace/models/doc_classify/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train doc_classify")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/doc_classify/")
...
def _build_model(device):
    """TODO: Replace with the real EfficientNet-B0 model."""
...
def main():
    ...
    print(f"[INFO] Starting training: doc_classify v{args.version}", flush=True)

"""Train passport_intl_ocr (国际护照识别（MRZ解析）).

Architecture: PP-OCRv4+MRZ parser

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/passport_intl_ocr/ \\
                    --output /workspace/models/passport_intl_ocr/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train passport_intl_ocr")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/passport_intl_ocr/")
    p.add_argument("--output",  default="/workspace/models/passport_intl_ocr/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real PP-OCRv4+MRZ parser model."""
    import torch.nn as nn
    return nn.Sequential(
        nn.Flatten(),
        nn.Linear(921600, 256),
        nn.ReLU(),
...
def main():
    ...
    print(f"[INFO] Starting training: passport_intl_ocr v{args.version}", flush=True)

"""Train image_dehaze (图像去雾).

Architecture: AOD-Net/MSBDN

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/image_dehaze/ \\
                    --output /workspace/models/image_dehaze/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train image_dehaze")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/image_dehaze/")
    p.add_argument("--output",  default="/workspace/models/image_dehaze/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real AOD-Net/MSBDN model."""
...
def main():
    ...
    print(f"[INFO] Starting training: image_dehaze v{args.version}", flush=True)

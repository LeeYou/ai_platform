"""Train id_card_front_detect (身份证人像面检测).

Architecture: YOLOv8n

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/id_card_front_detect/ \\
                    --output /workspace/models/id_card_front_detect/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train id_card_front_detect")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/id_card_front_detect/")
    p.add_argument("--output",  default="/workspace/models/id_card_front_detect/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real YOLOv8n model."""
...
def main():
    ...
    print(f"[INFO] Starting training: id_card_front_detect v{args.version}", flush=True)

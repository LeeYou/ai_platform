"""Export deepfake_detect (深度伪造检测（DeepFake检测）) model to ONNX format.

Usage:
    python export.py --output /workspace/models/deepfake_detect/v1.0.0/ --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Export deepfake_detect to ONNX")
...
def _write_preprocess_json(output_path: str) -> None:
    data = {
        "resize": {"width": 299, "height": 299, "keep_ratio": True},
...
def _write_labels_json(output_path: str) -> None:
    data = {"labels": ['real', 'fake'], "num_classes": 2}
...
def main():
    ...
    # TODO: replace with real EfficientNet-B4/Xception model class

"""Train voice_liveness (语音活体检测).

Architecture: AASIST/RawBoost

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/voice_liveness/ \\
                    --output /workspace/models/voice_liveness/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train voice_liveness")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/voice_liveness/")
    p.add_argument("--output",  default="/workspace/models/voice_liveness/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real AASIST/RawBoost model."""
...
def main():
    ...
    print(f"[INFO] Starting training: voice_liveness v{args.version}", flush=True)

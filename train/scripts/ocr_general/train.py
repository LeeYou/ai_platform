"""Train ocr_general (通用文字识别).

Architecture: PaddleOCR/PP-OCRv4

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/ocr_general/ \\
                    --output /workspace/models/ocr_general/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train ocr_general")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/ocr_general/")
    p.add_argument("--output",  default="/workspace/models/ocr_general/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real PaddleOCR/PP-OCRv4 model."""
...
def main():
    ...
    print(f"[INFO] Starting training: ocr_general v{args.version}", flush=True)

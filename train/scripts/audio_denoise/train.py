"""Train audio_denoise (语音降噪).

Architecture: SEGAN/Demucs

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/audio_denoise/ \\
                    --output /workspace/models/audio_denoise/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train audio_denoise")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/audio_denoise/")
    p.add_argument("--output",  default="/workspace/models/audio_denoise/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real SEGAN/Demucs model."""
...
def main():
    ...
    print(f"[INFO] Starting training: audio_denoise v{args.version}", flush=True)

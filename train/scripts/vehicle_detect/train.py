"""Train vehicle_detect (车辆检测).

Architecture: YOLOv8s

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/vehicle_detect/ \\
                    --output /workspace/models/vehicle_detect/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train vehicle_detect")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/vehicle_detect/")
    p.add_argument("--output",  default="/workspace/models/vehicle_detect/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real YOLOv8s model."""
...
def main():
    ...
    print(f"[INFO] Starting training: vehicle_detect v{args.version}", flush=True)

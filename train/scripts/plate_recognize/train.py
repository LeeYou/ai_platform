"""Train plate_recognize (车牌识别).

Architecture: LPRNet/YOLOv8+CRNN

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/plate_recognize/ \\
                    --output /workspace/models/plate_recognize/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train plate_recognize")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/plate_recognize/")
    p.add_argument("--output",  default="/workspace/models/plate_recognize/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real LPRNet/YOLOv8+CRNN model."""
...
def main():
    ...
    print(f"[INFO] Starting training: plate_recognize v{args.version}", flush=True)

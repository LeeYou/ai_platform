"""Train bill_verify (票据验真).

Architecture: CNN+特征检测

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/bill_verify/ \\
                    --output /workspace/models/bill_verify/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train bill_verify")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/bill_verify/")
    p.add_argument("--output",  default="/workspace/models/bill_verify/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real CNN+特征检测 model."""
...
def main():
    ...
    print(f"[INFO] Starting training: bill_verify v{args.version}", flush=True)

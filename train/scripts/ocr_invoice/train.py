"""Train ocr_invoice (发票OCR).

Architecture: PP-OCRv4+LayoutParser

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/ocr_invoice/ \\
                    --output /workspace/models/ocr_invoice/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train ocr_invoice")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/ocr_invoice/")
    p.add_argument("--output",  default="/workspace/models/ocr_invoice/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real PP-OCRv4+LayoutParser model."""
...
def main():
    ...
    print(f"[INFO] Starting training: ocr_invoice v{args.version}", flush=True)

...
Architecture: Whisper-small/Paraformer

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/asr/ \\
                    --output /workspace/models/asr/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train asr")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/asr/")
    p.add_argument("--output",  default="/workspace/models/asr/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real Whisper-small/Paraformer model."""
...
def main():
    ...
    print(f"[INFO] Starting training: asr v{args.version}", flush=True)

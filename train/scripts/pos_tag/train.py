...
Architecture: BERT-CRF/LTP

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/pos_tag/ \\
                    --output /workspace/models/pos_tag/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train pos_tag")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/pos_tag/")
    p.add_argument("--output",  default="/workspace/models/pos_tag/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real BERT-CRF/LTP model."""
...
def main():
    ...
    print(f"[INFO] Starting training: pos_tag v{args.version}", flush=True)

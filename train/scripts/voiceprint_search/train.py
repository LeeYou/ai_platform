"""Train voiceprint_search (声纹检索（1:N检索）).
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train voiceprint_search")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/voiceprint_search/")
...
def _build_model(device):
    """TODO: Replace with the real ECAPA-TDNN model."""
...
def main():
    ...
    print(f"[INFO] Starting training: voiceprint_search v{args.version}", flush=True)

"""Export vehicle_attribute (车辆属性识别（颜色/车型/品牌）) model to ONNX format.

Usage:
    python export.py --output /workspace/models/vehicle_attribute/v1.0.0/ --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Export vehicle_attribute to ONNX")
    p.add_argument("--output",     default="/workspace/models/vehicle_attribute/v1.0.0/")
...
def _write_labels_json(output_path: str) -> None:
    data = {"labels": ['color', 'type', 'brand'], "num_classes": 3}
...
def main():
    ...
    # TODO: replace with real ResNet50 multi-head model class

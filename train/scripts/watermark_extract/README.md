...
DWT + CNN 在变换域提取隐式水印，WDNet / LaMa 去除可见水印，保留图像质量。
...
将数据集放置于 `/workspace/datasets/watermark_extract/` 目录，并按以下结构组织：

```
/workspace/datasets/watermark_extract/
├── train/
├── val/
└── test/
```
...
```bash
python export.py \
    --weights /workspace/models/watermark_extract/v1.0.0/best.pt \
    --format onnx
```
...
报告水印提取准确率（Bit Accuracy）；去水印任务报告 PSNR/SSIM；测试多种水印类型（文字/Logo/盲水印）。

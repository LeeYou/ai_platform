"""Train panoptic_segment (全景分割).

Architecture: Panoptic-FPN/Mask2Former

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/panoptic_segment/ \\
                    --output /workspace/models/panoptic_segment/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train panoptic_segment")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/panoptic_segment/")
    p.add_argument("--output",  default="/workspace/models/panoptic_segment/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real Panoptic-FPN/Mask2Former model."""
...
def main():
    ...
    print(f"[INFO] Starting training: panoptic_segment v{args.version}", flush=True)

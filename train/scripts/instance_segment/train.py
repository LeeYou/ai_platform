"""Train instance_segment (实例分割).

Architecture: Mask R-CNN/YOLOv8-seg

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/instance_segment/ \\
                    --output /workspace/models/instance_segment/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train instance_segment")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/instance_segment/")
    p.add_argument("--output",  default="/workspace/models/instance_segment/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real Mask R-CNN/YOLOv8-seg model."""
...
def main():
    ...
    print(f"[INFO] Starting training: instance_segment v{args.version}", flush=True)

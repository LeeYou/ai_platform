"""Train person_detect (人体检测).

Architecture: YOLOv8n

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/person_detect/ \\
                    --output /workspace/models/person_detect/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train person_detect")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/person_detect/")
    p.add_argument("--output",  default="/workspace/models/person_detect/v1.0.0/")
...
def main():
    ...
    print(f"[INFO] Starting training: person_detect v{args.version}", flush=True)

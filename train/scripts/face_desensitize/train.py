"""Train face_desensitize (人脸脱敏/像素化).

Architecture: MTCNN+pixel blur

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/face_desensitize/ \\
                    --output /workspace/models/face_desensitize/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train face_desensitize")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/face_desensitize/")
...
def _build_model(device):
    """TODO: Replace with the real MTCNN+pixel blur model."""
...
def main():
    ...
    print(f"[INFO] Starting training: face_desensitize v{args.version}", flush=True)

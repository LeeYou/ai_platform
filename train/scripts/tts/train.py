...
Architecture: VITS/FastSpeech2

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/tts/ \\
                    --output /workspace/models/tts/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train tts")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/tts/")
    p.add_argument("--output",  default="/workspace/models/tts/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real VITS/FastSpeech2 model."""
...
def main():
    ...
    print(f"[INFO] Starting training: tts v{args.version}", flush=True)

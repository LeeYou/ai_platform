"""Train social_security_ocr (社保卡识别).

Architecture: PP-OCRv4+KIE

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/social_security_ocr/ \\
                    --output /workspace/models/social_security_ocr/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train social_security_ocr")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/social_security_ocr/")
    p.add_argument("--output",  default="/workspace/models/social_security_ocr/v1.0.0/")
...
def main():
    ...
    print(f"[INFO] Starting training: social_security_ocr v{args.version}", flush=True)

"""Train keyword_extract (关键词提取).

Architecture: TextRank/KeyBERT

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/keyword_extract/ \\
                    --output /workspace/models/keyword_extract/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train keyword_extract")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/keyword_extract/")
    p.add_argument("--output",  default="/workspace/models/keyword_extract/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real TextRank/KeyBERT model."""
...
def main():
    ...
    print(f"[INFO] Starting training: keyword_extract v{args.version}", flush=True)

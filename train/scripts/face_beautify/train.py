"""Train face_beautify (人脸美颜/美型).

Architecture: UNET/Pix2Pix

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/face_beautify/ \\
                    --output /workspace/models/face_beautify/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train face_beautify")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/face_beautify/")
    p.add_argument("--output",  default="/workspace/models/face_beautify/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real UNET/Pix2Pix model."""
...
def main():
    ...
    print(f"[INFO] Starting training: face_beautify v{args.version}", flush=True)

"""Train ocr_table (表格识别与结构化提取).

Architecture: TableFormer/LORE

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/ocr_table/ \\
                    --output /workspace/models/ocr_table/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train ocr_table")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/ocr_table/")
    p.add_argument("--output",  default="/workspace/models/ocr_table/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real TableFormer/LORE model."""
...
def main():
    ...
    print(f"[INFO] Starting training: ocr_table v{args.version}", flush=True)

"""Train action_recognize (行为识别（摔倒/奔跑/打架等）).

Architecture: SlowFast/VideoSwin

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/action_recognize/ \\
                    --output /workspace/models/action_recognize/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train action_recognize")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/action_recognize/")
    p.add_argument("--output",  default="/workspace/models/action_recognize/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real SlowFast/VideoSwin model."""
...
def main():
    ...
    print(f"[INFO] Starting training: action_recognize v{args.version}", flush=True)

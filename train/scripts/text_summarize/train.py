"""Train text_summarize (文本摘要).

Architecture: BART/ProphetNet

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/text_summarize/ \\
                    --output /workspace/models/text_summarize/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train text_summarize")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/text_summarize/")
    p.add_argument("--output",  default="/workspace/models/text_summarize/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real BART/ProphetNet model."""
...
def main():
    ...
    print(f"[INFO] Starting training: text_summarize v{args.version}", flush=True)

"""Train product_recognize (商品识别).

Architecture: EfficientNet-B3

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/product_recognize/ \\
                    --output /workspace/models/product_recognize/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train product_recognize")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/product_recognize/")
    p.add_argument("--output",  default="/workspace/models/product_recognize/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real EfficientNet-B3 model."""
...
def main():
    ...
    print(f"[INFO] Starting training: product_recognize v{args.version}", flush=True)

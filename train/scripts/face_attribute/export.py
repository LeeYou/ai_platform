"""Export face_attribute (人脸属性识别（年龄/性别/口罩等）) model to ONNX format.

Usage:
    python export.py --output /workspace/models/face_attribute/v1.0.0/ --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Export face_attribute to ONNX")
...
def _write_preprocess_json(output_path: str) -> None:
    data = {
        "resize": {"width": 112, "height": 112, "keep_ratio": True},
...
def _write_labels_json(output_path: str) -> None:
    data = {"labels": ['no_mask', 'mask', 'hat', 'glasses', 'occluded', 'frontal'], "num_classes": 6}
...
def main():
    ...
    # TODO: replace with real MobileNetV3 multi-head model class

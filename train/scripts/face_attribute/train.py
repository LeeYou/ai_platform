"""Train face_attribute (人脸属性识别（年龄/性别/口罩等）).

Architecture: MobileNetV3 multi-head

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/face_attribute/ \\
                    --output /workspace/models/face_attribute/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train face_attribute")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/face_attribute/")
    p.add_argument("--output",  default="/workspace/models/face_attribute/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real MobileNetV3 multi-head model."""
...
def main():
    ...
    print(f"[INFO] Starting training: face_attribute v{args.version}", flush=True)

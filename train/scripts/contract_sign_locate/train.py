"""Train contract_sign_locate (合同签名位置识别).

Architecture: YOLOv8n

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/contract_sign_locate/ \\
                    --output /workspace/models/contract_sign_locate/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train contract_sign_locate")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/contract_sign_locate/")
    p.add_argument("--output",  default="/workspace/models/contract_sign_locate/v1.0.0/")
...
def main():
    ...
    print(f"[INFO] Starting training: contract_sign_locate v{args.version}", flush=True)

"""Train speech_emotion (语音情绪识别).

Architecture: HuBERT+分类头

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/speech_emotion/ \\
                    --output /workspace/models/speech_emotion/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train speech_emotion")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/speech_emotion/")
...
def _build_model(device):
    """TODO: Replace with the real HuBERT+分类头 model."""
...
def main():
    ...
    print(f"[INFO] Starting training: speech_emotion v{args.version}", flush=True)

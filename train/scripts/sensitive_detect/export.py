"""Export sensitive_detect (敏感信息识别（涉政/色情/暴恐）) model to ONNX format.

Usage:
    python export.py --output /workspace/models/sensitive_detect/v1.0.0/ --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Export sensitive_detect to ONNX")
    p.add_argument("--output",     default="/workspace/models/sensitive_detect/v1.0.0/")
...
def _write_preprocess_json(output_path: str) -> None:
    data = {
        "resize": {"width": 512, "height": 1, "keep_ratio": True},
...
def _write_labels_json(output_path: str) -> None:
    data = {"labels": ['clean', 'political', 'pornographic', 'violent'], "num_classes": 4}
...
def main():
    ...
    # TODO: replace with real RoBERTa+多标签分类 model class

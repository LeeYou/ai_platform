"""Train face_landmark (人脸关键点检测).

Architecture: PFLD/HRNet-W18

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/face_landmark/ \\
                    --output /workspace/models/face_landmark/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train face_landmark")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/face_landmark/")
    p.add_argument("--output",  default="/workspace/models/face_landmark/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real PFLD/HRNet-W18 model."""
...
def main():
    ...
    print(f"[INFO] Starting training: face_landmark v{args.version}", flush=True)

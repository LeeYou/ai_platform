"""Train ocr_driver_license (驾驶证识别).

Architecture: PP-OCRv4+KIE

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/ocr_driver_license/ \\
                    --output /workspace/models/ocr_driver_license/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train ocr_driver_license")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/ocr_driver_license/")
    p.add_argument("--output",  default="/workspace/models/ocr_driver_license/v1.0.0/")
...
def main():
    ...
    print(f"[INFO] Starting training: ocr_driver_license v{args.version}", flush=True)

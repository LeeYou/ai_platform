"""Train video_condense (视频浓缩).

Architecture: 活动检测+关键帧提取

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/video_condense/ \\
                    --output /workspace/models/video_condense/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train video_condense")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/video_condense/")
    p.add_argument("--output",  default="/workspace/models/video_condense/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real 活动检测+关键帧提取 model."""
...
def main():
    ...
    print(f"[INFO] Starting training: video_condense v{args.version}", flush=True)

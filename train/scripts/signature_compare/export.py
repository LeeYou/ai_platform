"""Export signature_compare (手写签名比对) model to ONNX format.

Usage:
    python export.py --output /workspace/models/signature_compare/v1.0.0/ --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Export signature_compare to ONNX")
    p.add_argument("--output",     default="/workspace/models/signature_compare/v1.0.0/")
...
def _write_preprocess_json(output_path: str) -> None:
    data = {
        "resize": {"width": 155, "height": 220, "keep_ratio": True},
...
def _write_labels_json(output_path: str) -> None:
    data = {"labels": ['same', 'different'], "num_classes": 2}
...
def main():
    ...
    # TODO: replace with real Siamese-CNN/SigNet model class
    import torch.nn as nn
    model = nn.Sequential(
        nn.Flatten(),
        nn.Linear(102300, 256),
        nn.ReLU(),

...
YOLOv8n 针对人脸检测微调，支持多尺度、遮挡、侧脸场景。
...
将数据集放置于 `/workspace/datasets/face_detect/` 目录，并按以下结构组织：
...
```bash
python export.py \
    --weights /workspace/models/face_detect/v1.0.0/best.pt \
    --format onnx
```
...
在 WIDER FACE val easy/medium/hard 上报告 mAP；统计各尺寸人脸检测精度。

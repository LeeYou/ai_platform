"""Train contract_extract (合同关键信息抽取).

Architecture: LayoutLMv3/UIE-contract

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/contract_extract/ \\
                    --output /workspace/models/contract_extract/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train contract_extract")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/contract_extract/")
    p.add_argument("--output",  default="/workspace/models/contract_extract/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real LayoutLMv3/UIE-contract model."""
...
def main():
    ...
    print(f"[INFO] Starting training: contract_extract v{args.version}", flush=True)

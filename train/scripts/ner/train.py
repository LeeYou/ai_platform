"""Train ner (命名实体识别（人名/地名/机构名）).

Architecture: BERT-CRF/RoBERTa-large

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/ner/ \\
                    --output /workspace/models/ner/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train ner")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/ner/")
    p.add_argument("--output",  default="/workspace/models/ner/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real BERT-CRF/RoBERTa-large model."""
...
def main():
    ...
    print(f"[INFO] Starting training: ner v{args.version}", flush=True)

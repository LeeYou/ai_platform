"""Train gesture_recognize (手势识别).

Architecture: MobileNetV3+MediaPipe

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/gesture_recognize/ \\
                    --output /workspace/models/gesture_recognize/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train gesture_recognize")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/gesture_recognize/")
    p.add_argument("--output",  default="/workspace/models/gesture_recognize/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real MobileNetV3+MediaPipe model."""
    import torch.nn as nn
    return nn.Sequential(
        nn.Flatten(),
        nn.Linear(150528, 256),
        nn.ReLU(),
        nn.Linear(256, max(1, 6)),
    )
...
def main():
    ...
    print(f"[INFO] Starting training: gesture_recognize v{args.version}", flush=True)

"""Train object_detect (目标检测（通用物体检测）).

Architecture: YOLOv8s

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/object_detect/ \\
                    --output /workspace/models/object_detect/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train object_detect")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/object_detect/")
    p.add_argument("--output",  default="/workspace/models/object_detect/v1.0.0/")
...
def main():
    ...
    print(f"[INFO] Starting training: object_detect v{args.version}", flush=True)

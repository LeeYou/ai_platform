"""Train face_recognition (人脸识别（1:N检索）).

Architecture: ArcFace/CosFace+ResNet50

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/face_recognition/ \\
                    --output /workspace/models/face_recognition/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train face_recognition")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/face_recognition/")
    p.add_argument("--output",  default="/workspace/models/face_recognition/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real ArcFace/CosFace+ResNet50 model."""
...
def main():
    ...
    print(f"[INFO] Starting training: face_recognition v{args.version}", flush=True)

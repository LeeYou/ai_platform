"""Train doc_classify_rpa (文档分类（RPA）).

Architecture: EfficientNet-B0+OCR特征

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/doc_classify_rpa/ \\
                    --output /workspace/models/doc_classify_rpa/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train doc_classify_rpa")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/doc_classify_rpa/")
    p.add_argument("--output",  default="/workspace/models/doc_classify_rpa/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real EfficientNet-B0+OCR特征 model."""
...
def main():
    ...
    print(f"[INFO] Starting training: doc_classify_rpa v{args.version}", flush=True)

"""Train form_extract (表单信息抽取).

Architecture: LayoutLMv3/PP-Structure

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/form_extract/ \\
                    --output /workspace/models/form_extract/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train form_extract")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/form_extract/")
    p.add_argument("--output",  default="/workspace/models/form_extract/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real LayoutLMv3/PP-Structure model."""
...
def main():
    ...
    print(f"[INFO] Starting training: form_extract v{args.version}", flush=True)

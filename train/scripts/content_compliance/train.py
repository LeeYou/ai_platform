"""Train content_compliance (内容合规检测).

Architecture: RoBERTa+规则融合

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/content_compliance/ \\
                    --output /workspace/models/content_compliance/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train content_compliance")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/content_compliance/")
    p.add_argument("--output",  default="/workspace/models/content_compliance/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real RoBERTa+规则融合 model."""
...
def main():
    ...
    print(f"[INFO] Starting training: content_compliance v{args.version}", flush=True)

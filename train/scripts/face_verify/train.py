"""Train face_verify (人脸认证比对（1:1比对）).

Architecture: ArcFace+ResNet50

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/face_verify/ \\
                    --output /workspace/models/face_verify/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train face_verify")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/face_verify/")
    p.add_argument("--output",  default="/workspace/models/face_verify/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real ArcFace+ResNet50 model."""
...
def main():
    ...
    print(f"[INFO] Starting training: face_verify v{args.version}", flush=True)

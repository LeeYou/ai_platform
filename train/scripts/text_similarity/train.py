"""Train text_similarity (文本相似度计算).

Architecture: SimCSE/BGE

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/text_similarity/ \\
                    --output /workspace/models/text_similarity/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train text_similarity")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/text_similarity/")
    p.add_argument("--output",  default="/workspace/models/text_similarity/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real SimCSE/BGE model."""
...
def main():
    ...
    print(f"[INFO] Starting training: text_similarity v{args.version}", flush=True)

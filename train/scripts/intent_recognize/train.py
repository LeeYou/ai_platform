"""Train intent_recognize (意图识别).

Architecture: BERT+分类头

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/intent_recognize/ \\
                    --output /workspace/models/intent_recognize/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train intent_recognize")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/intent_recognize/")
    p.add_argument("--output",  default="/workspace/models/intent_recognize/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real BERT+分类头 model."""
...
def main():
    ...
    print(f"[INFO] Starting training: intent_recognize v{args.version}", flush=True)

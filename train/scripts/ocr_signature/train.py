"""Train ocr_signature (手写签字OCR).

Architecture: TrOCR fine-tuned

Usage:
    python train.py --config config.json \\
                    --dataset /workspace/datasets/ocr_signature/ \\
                    --output /workspace/models/ocr_signature/v1.0.0/ \\
                    --version 1.0.0
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train ocr_signature")
    p.add_argument("--config",  default="config.json")
    p.add_argument("--dataset", default="/workspace/datasets/ocr_signature/")
    p.add_argument("--output",  default="/workspace/models/ocr_signature/v1.0.0/")
...
def _build_model(device):
    """TODO: Replace with the real TrOCR fine-tuned model."""
...
def main():
    ...
    print(f"[INFO] Starting training: ocr_signature v{args.version}", flush=True)
